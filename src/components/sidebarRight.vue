<template>
	<div id="layout-config" :class="containerClass">
		...
		<div class="layout-config-content">
			...
			<h5>Themes</h5>
			<h6 class="mt-0">Default</h6>
			<div class="grid free-themes">
				<div class="col-3 text-center">
					<button class="p-link" type="button" @click="changeTheme($event, '_default', false)">
						<img class="block" :src="$store.state.master.logo_app" alt="Bootstrap Light Blue" />
					</button>
				</div>
			</div>

			<h6 class="">Bootstrap</h6>
			<div class="grid free-themes">
				<div class="col-3 text-center">
...
import EventBus from '../AppEventBus';
	...
	export default {
		...
		mounted() {
			...
			// console.log(t)
			if(this.$store.state.theme.theme) this.changeTheme(false, t.theme, t.dark)
			else EventBus.on('theme-change', this.themeChangeListener);
		},
		methods: {
			...
			incrementScale() {
				this.scale++;
				this.applyScale();EventBus
			},
			applyScale() {
				document.documentElement.style.fontSize = this.scale + 'px';
			},
			changeTheme(event, theme, dark) {
				this.$store.commit('set_theme', {event, theme, dark})
